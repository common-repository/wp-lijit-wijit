...
Plugin URI: http://www.lijit.com/help/gettingstarted#installWordPress
...
Version: 1.1
Author: Lijit Networks Inc. and Crowd Favorite
Author URI: http://www.lijit.com
*/

	/**
	 * Copyright (c) 2009 Lijit Networks, Inc. All rights reserved.
	 *
...
function lwp_hijack_submit(e) {
	...
	function lijit_std_widget($args) {
		...
		echo '<script type="text/javascript" src="http://www.lijit.com/wijitinit?uri=http%3A%2F%2Fwww.lijit.com%2Fusers%2F'.
...
jQuery('window').ready(function(){
	 ...
	 *
	 *NEW code method is to request lijit.com/api/json/profiles/<USERNAME> , and parse returned json
	 * 
	 */
	function lijit_check_valid_user($username=false) {
		
		if(!$username) { $username = get_option('lijit_username'); }
		$results = lwp_Request('http://www.lijit.com/api/json/profiles/'.$username);
		if($results) {
			
			if (strstr($results,'"success":true,'))
			{
		...
		if($search_type == 'wijit') {
			...
			echo '				 	<p><a href="#" onclick="show_widget_code(\'wijit_code_sample\'); return false;">Click here</a> if your theme is not Widget enabled or if you want to manually  
									...
									<span class="manual_code" id="wijit_code_sample" style="display: none;"><span>
										...
										<pre>&lt;script type="text/javascript" src="http://www.lijit.com/wijitinit?uri=http%3A%2F%2Fwww.lijit.com%2Fusers%2F'.$username.'&amp;js=1"&gt;&lt;/script&gt;
&lt;a style="color: #999" href="http://www.lijit.com" id="lijit_wijit_pvs_link"&gt;Lijit Search&lt;/a&gt;</pre>
	...
	function lwp_Request($uri)
	{
		if (function_exists('wp_remote_request'))
		{
			$resp = wp_remote_request($uri);
			if (is_wp_error($resp))
			{
				return false;
			}
